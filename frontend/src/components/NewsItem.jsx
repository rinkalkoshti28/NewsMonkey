...
const NewsItem = (props) => {
  ...
  return (
    <>
      <div className="card w-100 h-110 p-4 shadow-xl relative">
          ...
          <div className="px-3">
            <h3 className="font-bold my-2 w-full h-10">{title ? title.length > 55 ? `${title.slice(0, 55)}...` : title : "No title available"}</h3>
            <p className="mb-2 w-full h-fit line-clamp-3">{description ? description : "No description available"}</p>
            <p className="text-sm text-gray-500 my-1">By {author ? author : "Unknown Author"}</p>
          </div>
          <a
